%% Allen and Oxenham Track, for modeling & psychophysical
% Johanna Fritzinger
% 7/6/2020
...
condition = 2;      % 1 = timbre, 2 = pitch
rove = 2;           % 1 = no rove, 2 = rove
DL_variation = 2;   % variation in non-target dimension (1-8, look at DL_var_val)

% Timbre condition
delta_Fc = 0.25;    % 200% CF
Fc_nom = 1200;      % Hz, nominal spectral centroid

% Pitch condition
delta_F0 = 0.1;     % 10% F0
F0_nom = 200;       % Hz, nominal f0
    ...
    for con = condition
        for r = rove
            for DL_var = DL_variation
                ...
                direction = [1 1];  % records direction changes
                ...
                while nreversals < 8
                    ...
                    % Psychophysical experiment & DV 
                    if con == 1 % timbre
                        DV_human(reps) = play_experiment(stimulus, Fs, Fc_order);
                        [DV_model(reps), est_Fc(reps,:)] = spec_centroid_model_mod(condition, stim, Fc_order);
                    else % pitch experiment
                        DV_human(reps) = play_experiment(stimulus, Fs, F0_order);
                        [DV_model(reps), est_Fc(reps,:)] = spec_centroid_model_mod(condition, stim, F0_order);
                    end               
                    
                    if DV_human(reps) == 1 % correct trial, could change this to DV_model to hear how model does
                        ncorrect = ncorrect+1;
