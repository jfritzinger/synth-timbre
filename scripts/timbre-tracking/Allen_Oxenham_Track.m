...
% Runs two-down one-up tracking based on Allen & Oxenham 2014 - can run the
% model but can also run psychophysical experiments
% J. Fritzinger, updated 12/1/2020
        ...
        for r = rove
            for DL_var = DL_variation
                ...
                while nreversals < 8
                    ...
                    %DV_human(reps) = runExperiment(stimulus, Fs, Fc_order);
                    [DV_model(reps), est_Fc(reps,:)] = runModel(condition, stim, Fc, F0, Fc_order, F0_order, plt);
                ...
                start_ind = find(reversal_tracker);
                 thresh(n_reps, DL_var) = mean(percentage(start_ind(3):start_ind(end)));
                
                % path = ...
                %file = ['C:\Users\jfritzinger\Box\03 - Timbre Modeling\7 - Model\Results\Cond_' num2str(con) '_Rep_' num2str(n_reps) '_DLvar_' num2str(DL_var)];
                %save(file)
            end
        end
    end
