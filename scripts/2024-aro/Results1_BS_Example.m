...
% R025S593, TT3N1, Good, CF = 2000Hz, BS
% R027S010, TT3N1, Good, CF = 770Hz, BS
clear
...
for ineuron = 1:2
	...
	switch ineuron
		case 1
			% R024S470, TT2N1, Excellent, CF = 1165Hz, BS
			session = 'R024S470_TT2_N1';
			CF = 1165;
		case 2

			% R024S450, TT2N1, Good, CF = 1718Hz, BS
			% session = 'R024S450_TT2_N1';
			% CF = 1718;

			% R024S473, TT2N1, Good, CF = 1150Hz, BS
			% session = 'R024S473_TT2_N1';
			% CF = 1150;

			% R024S478, TT2N1, Good, CF = 1320Hz, BS
	...
	x_label = [200 500 1000 2000 3000 4000 6000 8000];
	...
	for ind = 1:3
		...
		grid on
		hold off
		ylim([0 max(data_RM.rates, [], 'all')+15])
		set(gca,'XTick',[])
	...
	for ind = 1:num_ds
		annotation('textbox',[0.525 0.82-0.051*(ind-1) 0.0829 0.0869],...
			'Color',data_colors{ind},...
			'String',[num2str(params_ST{order(ind)}.spl+3) ' dB SPL'],...
			'FontSize',16,...
			'EdgeColor','none');
	end
	...
	hold on
	for ind = 1:num_ds
		[rate, ~] = plotST(params_ST{order(ind)}, SFIE{order(ind)}.average_ic_sout_BS, 0);
	...
	% R^2 annotation
	for ind = 1:num_ds
		annotation('textbox',[0.903 0.82-0.051*(ind-1) 0.082 0.0917],...
