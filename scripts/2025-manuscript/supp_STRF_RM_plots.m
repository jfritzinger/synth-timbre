...
% 
% 		if ispl == 1 && ibin == 1
% 			has_data = cellfun(@(s) contains(s, 'R'), sessions.ST_63dB_con)...
% 				& cellfun(@(s) contains(s, 'R'), sessions.STRF_con);
% 		elseif ispl == 1 && ibin == 2
% 			has_data = cellfun(@(s) contains(s, 'R'), sessions.ST_63dB) &...
% 				cellfun(@(s) contains(s, 'R'), sessions.STRF);
% 		elseif ispl == 2 && ibin == 1
% 			has_data = cellfun(@(s) contains(s, 'R'), sessions.ST_73dB_con)...
% 				& cellfun(@(s) contains(s, 'R'), sessions.STRF_con);
% 		elseif ispl == 2 && ibin == 2
% 			has_data = cellfun(@(s) contains(s, 'R'), sessions.ST_73dB) &...
% 				cellfun(@(s) contains(s, 'R'), sessions.STRF);
% 		end
% 		index = find(has_data);
% 
...
% 
% 			save(fullfile(datapath, 'STRF_Models', 'AllUnits_63_Bin.mat'),...
% 				"R2_STRF", 'R2_RM')
% 		end
% 	end
% end

%% Save data for box plots 
...
% 			CFs(ispl, isesh) = CF;
% 			save(fullfile(datapath, 'RM_predictions', 'AllUnits_Bin.mat'),...
% 				"R2", 'CF')
% 		end
% 	end
% end