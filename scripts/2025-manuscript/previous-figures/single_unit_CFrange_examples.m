...
for ineuron = 1:3
	...
	label_ind = 1;
	for ind = 1:3 %num_ds
		h(loc(ind, ineuron)) = subplot(3, 3, loc(ind, ineuron));
		j = j+1;
		hold on
...
left = linspace(0.12, 0.72, 3);
bottom = fliplr(linspace(0.1, 0.64, 3));
width = 0.26;
height = 0.27;

col = repmat(left, 1, 3);
row = reshape(repmat(bottom, 3, 1), 9, 1);
...
exportgraphics(gcf, fullfile(savepath, 'manuscript', 'examples-differentCFs.png'), 'Resolution', 600)
