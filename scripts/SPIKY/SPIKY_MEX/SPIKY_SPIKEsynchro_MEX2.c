...
#define uspikes_in prhs[2]
    ...
    float *uspikes1, *uspikes2, tau[2][2], abso, mini;
    ...
    for(trac1 = 0; trac1 < *num_trains; ++trac1)
        for(trac2 = trac1+1; trac2 < *num_trains;  ++trac2) {
            ...
            /*reallocate*/
            synchroPrReallocated = mxCreateLogicalArray(0, 0);
            mxSetM(synchroPrReallocated, 1);
            mxSetN(synchroPrReallocated, totalCounter);
            mxSetData(synchroPrReallocated, mxMalloc(sizeof(bool) * (totalCounter)));
            
            synchroReallocated = (bool *)mxGetPr(synchroPrReallocated);
            for(cntr1 = 0; cntr1 < totalCounter; cntr1++)
                synchroReallocated[cntr1] = synchro[cntr1];
            
            temp = (*num_trains - 1)**num_trains/2 - (*num_trains - 1 - trac1)*(*num_trains - trac1)/2 + trac2 - trac1 - 1;
            
            mxSetCell(synchro_out, temp, synchroPrReallocated);
        }
    return;
}
