...
if m_para.memo_num_measures>0                                               % ##### disc & pico & pili #####
    ...
    if r_para.num_runs>1 || num_profiles>0
        ...
        if m_para.num_pili_measures>0
            ...
            if f_para.subplot_posi(m_para.ri_spike_pili)
                ...
                if m_para.num_pili_measures>0
                    ...
                    if f_para.subplot_posi([m_para.spike_eero_pili])
                        spike_diffs_eero_l(1,pili_prof_run_indy)=mean(shiftdim(m_res.pili_measures_mat(m_para.measure_indy(...
                            m_para.spike_eero_pili),f_para.select_pairs,pili_load_run_indy),1),1);
                    end
                    if f_para.subplot_posi([m_para.spike_neb_eero_pili])
                        spike_diffs_neb_eero_l(1,pili_prof_run_indy)=mean(shiftdim(m_res.pili_measures_mat(m_para.measure_indy(...
                            m_para.spike_neb_eero_pili),f_para.select_pairs,pili_load_run_indy),1),1);
                    end
                    if f_para.subplot_posi(m_para.realtime_spike_pili)
                        ...
                        if m_para.num_pili_measures>0
                            ...
                            if f_para.subplot_posi([m_para.spike_eero_pili])
                                spike_diffs_eero_l(gsgz,pili_prof_run_indy)=mean(shiftdim(m_res.pili_measures_mat(...
                                    m_para.measure_indy(m_para.spike_eero_pili),gm_indy,pili_load_run_indy),1),1);
                            end
                            if f_para.subplot_posi([m_para.spike_neb_eero_pili])
                                spike_diffs_neb_eero_l(gsgz,pili_prof_run_indy)=mean(shiftdim(m_res.pili_measures_mat(...
                                    m_para.measure_indy(m_para.spike_neb_eero_pili),gm_indy,pili_load_run_indy),1),1);
                            end
                            if f_para.subplot_posi(m_para.realtime_spike_pili)
...
if mod(f_para.plot_mode,2)>0
    ...
    if any(f_para.subplot_posi(m_para.bi_measures))
        ...
        if f_para.profile_norm_mode==1
            ...
            if f_para.subplot_posi(m_para.ri_spike_pili)
                m_para.maxbivals(m_para.ri_spike_pili)=max(max(spike_diffs_ri_l));
            end
            if f_para.subplot_posi(m_para.spikesync_disc)
...
% Stimulus    Spikes    PSTH   I     S  S_r S_f   SPIKE-Sync SPIKE-Order  RI-S S_N  S_E  S_NE    Sp   Sp_r Sp_f
%
...
measure_paras={...
    ...
    {'isi'; 'spike_diffs_ri_l';       [m_para.all_measures_str{m_para.ri_spike_pili},ab_str];     m_para.minbivals(m_para.ri_spike_pili); m_para.maxbivals(m_para.ri_spike_pili); 0;        2};...            % 10 RI-SPIKE
    {'isi'; 'spike_diffs_neb_l';          [m_para.all_measures_str{m_para.spike_neb_pili},ab_str];     m_para.minbivals(m_para.spike_neb_pili); m_para.maxbivals(m_para.spike_neb_pili); 0;        2};...            % 11 SPIKE-Neb
    {'isi'; 'spike_diffs_eero_l';          [m_para.all_measures_str{m_para.spike_eero_pili},ab_str];     m_para.minbivals(m_para.spike_eero_pili); m_para.maxbivals(m_para.spike_eero_pili); 0;        2};...            % 12 SPIKE-Eero
    {'isi'; 'spike_diffs_neb_eero_l';          [m_para.all_measures_str{m_para.spike_neb_eero_pili},ab_str];     m_para.minbivals(m_para.spike_neb_eero_pili); m_para.maxbivals(m_para.spike_neb_eero_pili); 0;        2};...  % 13 SPIKE-Neb-Eero

    {'isi';  'spike_diffs';          [m_para.all_measures_str{m_para.spike_pico},ab_str];           m_para.minbivals(m_para.spike_pico); m_para.maxbivals(m_para.spike_pico); 0;          1};...    % 14 SPIKE-pico
    {'isi';  'spike_diffs_realtime'; [m_para.all_measures_str{m_para.realtime_spike_pico},ab_str];  m_para.minbivals(m_para.realtime_spike_pico); m_para.maxbivals(m_para.realtime_spike_pico); 0; 1};...    % 15 SPIKE-realtime-pico
    {'isi';  'spike_diffs_forward';   [m_para.all_measures_str{m_para.forward_spike_pico},ab_str];    m_para.minbivals(m_para.forward_spike_pico); m_para.maxbivals(m_para.forward_spike_pico); 0;   1};...    % 16 SPIKE-forward-pico

    {'win_cum_isi2';  'spike_diffs_disc';  [m_para.all_measures_str{m_para.spike_disc},ab_str];    m_para.minbivals(m_para.spike_disc); m_para.maxbivals(m_para.spike_disc); 0;   3};...    %  17 SPIKE-Disc
    {'win_cum_isi2';  'spike_diffs_neb_disc';  [m_para.all_measures_str{m_para.spike_neb_disc},ab_str];    m_para.minbivals(m_para.spike_neb_disc); m_para.maxbivals(m_para.spike_neb_disc); 0;   3};...    %  18 SPIKE-Disc-Neb
    {'win_cum_isi2';  'spike_diffs_eero_disc';  [m_para.all_measures_str{m_para.spike_eero_disc},ab_str];    m_para.minbivals(m_para.spike_eero_disc); m_para.maxbivals(m_para.spike_eero_disc); 0;   3};...    %  19 SPIKE-Disc-Eero
    {'win_cum_isi2';  'spike_diffs_neb_eero_disc';  [m_para.all_measures_str{m_para.spike_neb_eero_disc},ab_str];    m_para.minbivals(m_para.spike_neb_eero_disc); m_para.maxbivals(m_para.spike_neb_eero_disc); 0;   3};...    %  20 SPIKE-Disc-Neb-Eero
};
%     X-input         Y-input                    Label                            Min-value                          Max-value                       Average Datatype
