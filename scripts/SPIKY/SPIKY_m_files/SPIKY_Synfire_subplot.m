if run==1
    ...
    cm=colormap;
    num_cols=size(cm,1);
    dcol_indy=round(num_cols:-(num_cols-1)/(para.num_trains-1):1);
    dcols=cm(dcol_indy,:);
    num_cols2=size(dcols,1);
...
if ((run==1 && mod(plotting,2)>0) || (run==2 && mod(plotting,16)>7))
    ...
    if run==1
        synf_title_fh(1)=title('Spike trains','Visible',para.synf_title_vis,'Color',para.synf_title_col,'FontSize',para.synf_title_fs,...
            'FontWeight',para.synf_title_fw,'FontAngle',para.synf_title_fa,'UIContextMenu',synf_title_cmenu);
    else
        synf_title_fh(7)=title('Sorted spike trains','Visible',para.synf_title_vis,'Color',para.synf_title_col,'FontSize',para.synf_title_fs,...
            'FontWeight',para.synf_title_fw,'FontAngle',para.synf_title_fa,'UIContextMenu',synf_title_cmenu);
    end
    if run==2 || plotting<2
        synf_label_fh(1)=xlabel('Time','Visible',para.synf_label_vis,'Color',para.synf_label_col,'FontSize',para.synf_label_fs,...
    ...
    if mod(color_spikes,2)>0
        ...
        if num_plots<3 && d_para.num_trains>10
            set(cb,'YTick',0:0.1:1,'YTickLabel',-1:0.2:1)
        elseif num_plots<5 && d_para.num_trains>4
            set(cb,'YTick',0:0.25:1,'YTickLabel',-1:0.5:1)
        else
            %set(cb,'YTick',1:(num_cols2-1)/2:num_cols2,'YTickLabel',-1:1)
            set(cb,'YTick',0:0.5:1,'YTickLabel',-1:1)
        end
        xl=xlim;
        yl=ylim;
        synf_cblabel_fh(run)=text(xl(1)+1.1*diff(xl),yl(2)-0.25*diff(yl),'D','Visible',para.synf_cblabel_vis,'Color',para.synf_cblabel_col,...
            'FontSize',para.synf_cblabel_fs,'FontWeight',para.synf_cblabel_fw,'FontAngle',para.synf_cblabel_fa,'UIContextMenu',synf_cblabel_cmenu);
        gcp=get(gca,'Position');
    end
    if patching==1
        if run==1
            %coli=ceil((st_indy+1.01)*31.5)
            %coli=ceil(((st_indy-1)/(para.num_trains-1))*(num_cols-1))+1
            for trc=1:para.num_trains
...
if ((run==1 && mod(plotting,4)>1) || (run==2 && mod(plotting,32)>15))
    ...
    if mod(plot_profiles,8)>3                                               % E-profile (symmetric, to be maximized)
        ...
        if run==1
            %text(xl(2)+0.01*(xl(2)-xl(1)),yl(1)+(0.75-(prof_pos(prc2)-1)*0.25-0.25*(mod(plot_profiles,4)<=1))*(yl(2)-yl(1)),[prof_letter(3),'_u=',...
            %num2str(mean_thr,3)],'Color',prof_color(1),'FontSize',fs+1,'FontWeight','bold')
            synf_f_result_fh(run)=text(xl(2)+0.01*(xl(2)-xl(1)),yl(1)+(0.75-(prof_pos(prc2)-1)*0.25-0.25*(mod(plot_profiles,4)<=1))*(yl(2)-yl(1)),[prof_letter(3),...
                '_u=',num2str(mean_thr,3)],'Visible',para.synf_f_result_vis,'Color',para.synf_f_result_col,'FontSize',para.synf_f_result_fs,...
                'FontWeight',para.synf_f_result_fw,'FontAngle',para.synf_f_result_fa,'UIContextMenu',synf_f_result_cmenu);
        else
            %text(xl(2)+0.01*(xl(2)-xl(1)),yl(1)+(0.75-(prof_pos(prc2)-1)*0.25-0.25*(mod(plot_profiles,4)<=1))*(yl(2)-yl(1)),[prof_letter(3),'_s=',...
            %num2str(mean_thr,3)],'Color',prof_color(1),'FontSize',fs+1,'FontWeight','bold')
            synf_f_result_fh(run)=text(xl(2)+0.01*(xl(2)-xl(1)),yl(1)+(0.75-(prof_pos(prc2)-1)*0.25-0.25*(mod(plot_profiles,4)<=1))*(yl(2)-yl(1)),...
                [prof_letter(3),'_s=',num2str(mean_thr,3)],'Visible',para.synf_f_result_vis,'Color',para.synf_f_result_col,'FontSize',para.synf_f_result_fs,...
                'FontWeight',para.synf_f_result_fw,'FontAngle',para.synf_f_result_fa,'UIContextMenu',synf_f_result_cmenu);
        end
    end
    ...
    if mod(plot_profiles,2)>0                                               % SPIKE-Sync-profile C (symmetric envelope)
        ...
        synf_c_result_fh(run)=text(xl(2)+0.01*(xl(2)-xl(1)),yl(1)+(0.75-(prof_pos(prc2)-1)*0.25)*(yl(2)-yl(1)),[prof_letter(1),'=',num2str(mean_y3,3)],...
            'Visible',para.synf_c_result_vis,'Color',para.synf_c_result_col,'FontSize',para.synf_c_result_fs,'FontWeight',para.synf_c_result_fw,...
            'FontAngle',para.synf_c_result_fa,'UIContextMenu',synf_c_result_cmenu);
        %text(xl(2)+0.01*(xl(2)-xl(1)),yl(1)+(0.75-(prof_pos(prc2)-1)*0.25)*(yl(2)-yl(1)),[prof_letter(1),'=',num2str(mean_y3,3)],'Color',prof_color(3),...
        %'FontSize',fs+1)
    end
    ...
    if mod(plot_profiles,4)>1                                               % D-profile (anti-symmetric, mostly useless)
        ...
        %text(xl(2)+0.01*(xl(2)-xl(1)),yl(1)+(0.75-(prof_pos(prc2)-1)*0.25)*(yl(2)-yl(1)),[prof_letter(2),'=',num2str(mean_y2,3)],'Color',prof_color(2),...
    ...
    for prc=1:3
        if profs(prc)>0
            prc2=prc2+1;
            if prc<3
                %text(xl(1)-0.05*(xl(2)-xl(1)),yl(1)+(0.8-(prof_pos(prc2)-1)*0.25)*(yl(2)-yl(1)),prof_letter(prc),'FontSize',fs-1,'Rotation',0,...
                %'Color',prof_color(prc),'FontWeight','bold')
            else
                %text(xl(1)-0.05*(xl(2)-xl(1)),yl(1)+(0.8-(prof_pos(prc2)-1)*0.25)*(yl(2)-yl(1)),prof_letter(prc),'FontSize',fs-1,'Rotation',0,...
                %'Color',prof_color(prc))
            end
        end
    end
    if plotting<4
    ...
    if mod(color_spikes,2)>0
        gcp2=get(gca,'Position');
        gcp2(3)=gcp(3);
        set(gca,'Position',gcp2);
    end
    if num_plots>1
        synf_sublabel_fh(plot_pos(subplotc))=text(xl(1)-0.06*(xl(2)-xl(1)),yl(2)+0.05*(yl(2)-yl(1)),['(',alphabet(plot_pos(subplotc)),')'],...
            'Visible',para.synf_sublabel_vis,'Color',para.synf_sublabel_col,'FontSize',para.synf_sublabel_fs,'FontWeight',para.synf_sublabel_fw,...
            'FontAngle',para.synf_sublabel_fa,'UIContextMenu',synf_sublabel_cmenu);
    ...
    if mark_upper_matrix_half==1
        for trc=1:para.num_trains-1
            synf_halfmat_line_lh(run,trc*2-1)=line((trc+0.5)*ones(1,2),[trc-0.5 trc+0.5],...
                'Visible',para.synf_halfmat_line_vis,'Color',para.synf_halfmat_line_col,'LineStyle',para.synf_halfmat_line_ls,'LineWidth',...
                para.synf_halfmat_line_lw,'UIContextMenu',synf_halfmat_line_cmenu);
        ...
        synf_halfmat_line_lh(run,para.num_trains*2-1)=line((para.num_trains+0.5)*ones(1,2),[0.5 para.num_trains-0.5],'Visible',para.synf_halfmat_line_vis,...
            'Color',para.synf_halfmat_line_col,'LineStyle',para.synf_halfmat_line_ls,'LineWidth',para.synf_halfmat_line_lw,'UIContextMenu',synf_halfmat_line_cmenu);
        synf_halfmat_line_lh(run,para.num_trains*2)=line([1.5 para.num_trains+0.5],0.5*ones(1,2),'Visible',para.synf_halfmat_line_vis,'Color',...
            para.synf_halfmat_line_col,'LineStyle',para.synf_halfmat_line_ls,'LineWidth',para.synf_halfmat_line_lw,'UIContextMenu',synf_halfmat_line_cmenu);
...
if run==2
    ...
    if num_surros>0
        ...
        surros_cum=SPIKY_Synfire_f_generate_surros(sto_profs,so_profs,all_trains,num_surros,dataset,event_seps,fin);          % ###################
        ...
        if mod(plotting,128)>63
            ...
            if show_z_scores==1
                ...
                line((mean_surros+std_surros)*ones(1,2),yl(1)+[0.6 0.7]*(yl(2)-yl(1)),'Color',cols(1),'LineWidth',3)
                if pval<=0.05
                    synf_title_fh(5)=title(['z = ',num2str(z_score,3),'  ;  p = ',num2str(pval,2),'**'],'Visible',para.synf_title_vis,'Color',para.synf_title_col,...
                        'FontSize',para.synf_title_fs,'FontWeight',para.synf_title_fw,'FontAngle',para.synf_title_fa,'UIContextMenu',synf_title_cmenu);
                elseif pval<0.3
                ...
                if pval<=0.05
                    synf_title_fh(5)=title(['Significance:  p = ',num2str(pval,2),'**'],'Visible',para.synf_title_vis,'Color',para.synf_title_col,...
                        'FontSize',para.synf_title_fs,'FontWeight',para.synf_title_fw,'FontAngle',para.synf_title_fa,'UIContextMenu',synf_title_cmenu);
                elseif pval<0.3
                    synf_title_fh(5)=title('Significance:  p > 0.05','Visible',para.synf_title_vis,'Color',para.synf_title_col,'FontSize',para.synf_title_fs,...
                        'FontWeight',para.synf_title_fw,'FontAngle',para.synf_title_fa,'UIContextMenu',synf_title_cmenu);
                else
                    synf_title_fh(5)=title('Significance:  p >> 0.05','Visible',para.synf_title_vis,'Color',para.synf_title_col,'FontSize',para.synf_title_fs,...
                        'FontWeight',para.synf_title_fw,'FontAngle',para.synf_title_fa,'UIContextMenu',synf_title_cmenu);
            ...
            synf_sublabel_fh(6)=text(xl(1)-0.5*(xl(2)-xl(1)),yl(2)+0.05*(yl(2)-yl(1)),['(',alphabet(6),')'],'Visible',para.synf_sublabel_vis,'Color',...
                para.synf_sublabel_col,'FontSize',para.synf_sublabel_fs,'FontWeight',para.synf_sublabel_fw,'FontAngle',para.synf_sublabel_fa,...
                'UIContextMenu',synf_sublabel_cmenu);
        end
