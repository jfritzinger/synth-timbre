...
if isfield(d_para,'matfile')
    ...
    if ~exist('spikes','var') || isempty(spikes) || ~((iscell(spikes) && length(spikes)>1 && isnumeric(spikes{1})) || (isnumeric(spikes) && ismatrix(spikes) ...
            && size(spikes,1)>1) || (iscell(spikes) && length(spikes)==1 && ismatrix(spikes{1}) && size(spikes{1},1)>1 && isnumeric(spikes{1})))
    ...
    if ~isempty(fieldnames(dummy))
        ...
        if exist('thick_markers','var')
            ...
            if isnumeric(dummy) && ismatrix(dummy) && max(size(dummy))==numel(dummy)
