...
for ruc=1 %:r_para.num_runs   % ruc=r_para.num_runs:-1:1
    ...
    for trac1=1:d_para.num_trains-1
        ...
        for trac2=trac1+1:d_para.num_trains
            ...
            if ruc==1
                ...
                if exist(['SPIKY_SPIKEsynchro_MEX2XXXX.',mexext],'file')  % ##### SPIKE-order not yet implemented in MEX, use Matlab-code #####
                    % #################################### check spikes on edges
                    if pac==1
    ...
    if ruc==1 && sum(num_spikes)>0 && min(cellfun(@(v) v(1), spikes(cellfun('length',spikes)>0)))>d_para.tmin
    ...
    if ruc==r_para.num_runs && sum(num_spikes)>0 && max(cellfun(@(v) v(end), spikes(cellfun('length',spikes)>0)))<d_para.tmax
...
ave_bi_vect=ave_bi_vect./repmat(num_pair_all_spikes+(num_pair_all_spikes==0),2,1);
...
for mac=1:2
    ...
    if mac==m_para.spike_sync_pili
        ...
        eval(['results.',char(m_para.all_measures_string(mac)),'.overall=all_distances(mac);'])                   % ############### 1-
    end
    eval(['results.',char(m_para.all_measures_string(mac)),...
    ...
    if mac==m_para.spike_order_pili
        ...
        eval(['results.',char(m_para.all_measures_string(mac)),...
            '.spike_number_maximum=2*sum(sort(num_spikes).*(d_para.num_trains-1:-1:0))/(sum(num_spikes)*(d_para.num_trains-1));'])
        eval(['results.',char(m_para.all_measures_string(mac)),...
            '.pairwise_maximum=sum(sum(abs(results.SPIKE_order.matrix_cum)))/(sum(num_spikes)*(d_para.num_trains-1));'])
