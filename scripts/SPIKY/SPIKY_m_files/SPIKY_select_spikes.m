...
%         [num2str(select_trains),repmat(blanks(25),length(select_trains),1),num2str(num_spikes)]
    ...
    function SPIKY_plot_histo(varargin)
        ...
        for trac=1:num_trains
            [histo_y,histo_x]=histogram([spikes{select_trains(trac)}],f_para.psth_num_bins);
        ...
        histo_y=histcounts([spikes{select_trains}],edges);                                                                  % #### check (see Baroni email) ####
        %[histo_y,histo_x]=histcounts([spikes{select_trains}],edges);
