...
set(SM1_UserData.fh,'Userdata',SM1_UserData)
    ...
    function SM1_Load_markers(varargin)
        ...
        if d_para.matfile~=0
            ...
            if ~isnumeric(SM1_UserData.thick_markers) || ~ismatrix(SM1_UserData.thick_markers) || ~any(size(SM1_UserData.thick_markers)==1)
...
% ########################################################
    ...
    function SM2_Load_markers(varargin)
        ...
        if d_para.matfile~=0
            ...
            if ~isnumeric(SM2_UserData.thin_markers) || ~ismatrix(SM2_UserData.thin_markers) || ~any(size(SM2_UserData.thin_markers)==1)
                if ~strcmp(variable,'A9ZB1YC8X')
                    set(0,'DefaultUIControlFontSize',16);
                    mbh=msgbox(sprintf('No vector has been chosen. Please try again!'),'Warning','warn','modal');
                    htxt=findobj(mbh,'Type','text');
