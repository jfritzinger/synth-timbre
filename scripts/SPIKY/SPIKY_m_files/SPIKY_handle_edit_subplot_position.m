...
uicontrol(Edit_pushbutton)
    ...
    function [] = Edit_pushbutton_callback(varargin)
        ...
        xpos_in=abs(str2num(regexprep(xpos_str_in,f_para.regexp_str_scalar_float,'')));
        if ~isempty(xpos_in)
        ...
        ypos_in=abs(str2num(regexprep(ypos_str_in,f_para.regexp_str_scalar_float,'')));
        if ~isempty(ypos_in)
        ...
        width_in=abs(str2num(regexprep(width_str_in,f_para.regexp_str_scalar_float,'')));
        if ~isempty(width_in)
        ...
        height_in=abs(str2num(regexprep(height_str_in,f_para.regexp_str_scalar_float,'')));
