...
set(SS1_UserData.fh,'Userdata',SS1_UserData)
    ...
    function SS1_Load_separators(varargin)
        ...
        if d_para.matfile~=0
            ...
            if ~isnumeric(SS1_UserData.thick_separators) || ~ismatrix(SS1_UserData.thick_separators) || ~any(size(SS1_UserData.thick_separators)==1)
...
% ########################################################
    ...
    function SS2_Load_separators(varargin)
        ...
        if d_para.matfile~=0
            ...
            if ~isnumeric(SS2_UserData.thin_separators) || ~ismatrix(SS2_UserData.thin_separators) || ~any(size(SS2_UserData.thin_separators)==1)
                if ~strcmp(variable,'A9ZB1YC8X')
                    set(0,'DefaultUIControlFontSize',16);
                    mbh=msgbox(sprintf('No vector has been chosen. Please try again!'),'Warning','warn','modal');
                    htxt=findobj(mbh,'Type','text');
