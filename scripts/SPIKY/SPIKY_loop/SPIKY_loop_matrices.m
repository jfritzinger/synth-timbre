                ...
                for pmatc=1:m_para.num_disc_measures
                    ...
                    for sac=1:f_para.num_selective_averages
                        for selc=1:f_para.num_sel_ave(sac)
                            ...
                            sel_indy=find(pfx>d_para.tmin & pfx>=f_para.selective_averages{sac}(2*selc-1) & pfx<=f_para.selective_averages{sac}(2*selc) & ...
                                pfx<d_para.tmax);
                            m_res.movie_vects_sa(matc2,1:f_para.num_select_pairs,sac)=m_res.movie_vects_sa(matc2,1:f_para.num_select_pairs,sac)+...
                                sum(pfy(:,sel_indy),2)';
                            if pmatc==1
                            ...
                            if ~isempty(trig_indy)
                                ...
                                if ismember(m_para.select_pili_measures(lmatc),m_para.spike_pili)       % regular
                                    m_res.movie_vects_ta(matc2,1:f_para.num_select_pairs,tac)=...
                                        ...
                                        (m_res.pili_supi(r_para.run_pili_starts(ruc)+tlind)-m_res.pili_supi(r_para.run_pili_starts(ruc)+tlind-1)),...
                                        f_para.num_select_pairs,1),-1),3)*m_res.movie_vects_ta_pili_weight(tac,ruc);
                                else                                                                            % realtime, forward
                                    m_res.movie_vects_ta(matc2,1:f_para.num_select_pairs,tac)=...
                                        m_res.movie_vects_ta(matc2,1:f_para.num_select_pairs,tac)+...
                                        mean( 1./(1./m_res.pili_measures_mat(matc3,f_para.select_pairs,tlind)+...
                                        (1./(m_res.pili_measures_mat(matc3,f_para.select_pairs,tlind+1)+...
                                        (m_res.pili_measures_mat(matc3,f_para.select_pairs,tlind+1)==0))-...
                                        1./(m_res.pili_measures_mat(matc3,f_para.select_pairs,tlind)+...
                                        (m_res.pili_measures_mat(matc3,f_para.select_pairs,tlind)==0))).*...
                                        shiftdim(repmat((f_para.triggered_averages{tac}(trig_indy)-m_res.pili_supi(r_para.run_pili_starts(ruc)+tlind-1))./...
                                        (m_res.pili_supi(r_para.run_pili_starts(ruc)+tlind)-m_res.pili_supi(r_para.run_pili_starts(ruc)+tlind-1)),...
                                        f_para.num_select_pairs,1),-1)) ,3)*m_res.movie_vects_ta_pili_weight(tac,ruc);
                                end
