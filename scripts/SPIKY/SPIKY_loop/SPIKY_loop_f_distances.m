...
m_para.all_measures_str={'I';'S';'RI_S';'S_r';'S_f';'C';'D';'PSTH'}; % 1:ISI,2:SPIKE,3:RI-SPIKE,4:realtimeSPIKE,5:forwardSPIKE,6:SPIKE-Sync,7:SPIKE-Order,8:PSTH
m_para.all_measures_string={'ISI';'SPIKE';'RI_SPIKE';'SPIKE_realtime';'SPIKE_forward';'SPIKE_synchro';'SPIKE_order';'PSTH'};
...
m_para.isi_pico=1;
m_para.spike_pili=2;
m_para.ri_spike_pili=3;
m_para.realtime_spike_pili=4;
m_para.forward_spike_pili=5;
m_para.spikesync_disc=6;
m_para.spikeorder_disc=7;
m_para.psth=8;
...
m_para.pili_measures=[m_para.spike_pili m_para.ri_spike_pili m_para.realtime_spike_pili m_para.forward_spike_pili];
...
if m_para.num_sel_bi_measures>0     % num_sel_measures>0
    ...
    for trac=1:d_para.num_trains
        start_dummy=[1 max([m_res.num_tmin_spikes 1])+find(all_trains(max([m_res.num_tmin_spikes 1])+1:end-max([m_res.num_tmax_spikes 1]))==trac)];
        % indices of spike train 'trac' in pooled spike train (spikes at the beginning of the ISI)
        end_dummy=[start_dummy(2:length(start_dummy))-1 m_res.num_all_isi-m_res.num_tmax_spikes+1];
        % indices of spike train 'trac' in pooled spike train (spikes at the end of the ISI)
        
        %len3=end_dummy-start_dummy+1-histc(empties,start_dummy)       % old version with histc
        %%len2=end_dummy-start_dummy+1-histcounts(empties,start_dummy); % gives error : Matrix dimensions must agree.
        len=end_dummy-start_dummy+1-histcounts(empties,[start_dummy inf]); % new version with histcount, corrected
        %len_fab=end_dummy-start_dummy+1-histcounts(empties,[start_dummy 2*start_dummy(end)-start_dummy(end-1)])
    ...
    %         start_dummy=[1 max([m_res.num_tmin_spikes 1])+find(all_trains(max([m_res.num_tmin_spikes 1])+1:end-max([m_res.num_tmax_spikes 1]))==trac)];
    %         end_dummy=[start_dummy(2:length(start_dummy))-1 m_res.num_all_isi-m_res.num_tmax_spikes+1];
    %         len=end_dummy-start_dummy+1-histcounts(empties,start_dummy);
    %         ive{trac}=cumsum(len);
    %         ivs{trac}=[1 ive{trac}(1:end-1)+1];
    %     end
    %     clear empties % all_trains
    ...
    if f_para.edge_correction
        ...
        if any(f_para.select_measures([m_para.spike_pili m_para.ri_spike_pili m_para.realtime_spike_pili]))
            prev_edge_cor_indy=cell(1,d_para.num_trains);
        end
        if any(f_para.select_measures([m_para.spike_pili m_para.ri_spike_pili m_para.forward_spike_pili]))
    ...
    for ruc=r_para.num_runs:-1:r_para.ruc_final
        ...
        if any(f_para.select_measures([m_para.spikesync_disc m_para.isi_pico m_para.spike_pili m_para.ri_spike_pili ]))
        ...
        if any(f_para.select_measures([m_para.spike_pili m_para.ri_spike_pili m_para.realtime_spike_pili]))  % SPIKE-Pre-Pico
            ...
            if any(f_para.select_measures([m_para.spike_pili m_para.ri_spike_pili m_para.realtime_spike_pili]))                     % SPIKE-Pre-pili
                prev_spikes_indy_pili=zeros(d_para.num_trains,r_para.run_pili_lengths(ruc),'uint32');
                % two values at start/end of each interval of pooled spike train: index of previous uspike in that spike train
                prev_spikes_pili=zeros(d_para.num_trains,r_para.run_pili_lengths(ruc));
                % two values at start/end of each interval of pooled spike train: distance to previous uspike in that spike train
                for trac=1:d_para.num_trains
        ...
        if any(f_para.select_measures([m_para.spike_pili m_para.ri_spike_pili m_para.forward_spike_pili]))  % SPIKE-forward-Pico
            ...
            if any(f_para.select_measures([m_para.spike_pili m_para.ri_spike_pili m_para.forward_spike_pili]))                       % SPIKE-forward-pili
                foll_spikes_indy_pili=zeros(d_para.num_trains,r_para.run_pili_lengths(ruc),'uint32');
                % two values at start/end of each interval of pooled spike train: index of following uspike in that spike train
                foll_spikes_pili=zeros(d_para.num_trains,r_para.run_pili_lengths(ruc));
                % two values at start/end of each interval of pooled spike train: distance to following uspike in that spike train
                for trac=1:d_para.num_trains
        ...
        if m_para.num_pili_measures>0
                ...
                if exist(['SPIKY_realtimeSPIKE_MEX.',mexext],'file')
                    ...
                    for trac1=1:d_para.num_trains-1
                        for trac2=trac1+1:d_para.num_trains
                            ...
                            start_dummy=trac1*(1-dummy)+trac2*dummy;   % index of spike train with earlier spike
                            end_dummy=trac2*(1-dummy)+trac1*dummy;   % index of spike train with later spike
                            for sc=1:r_para.run_pili_lengths(ruc)
                                m_res.pili_measures_mat(m_para.measure_indy(m_para.realtime_spike_pili),pac,sc)= ...
                                    ...
                                    run_udists{start_dummy(sc),end_dummy(sc)}(prev_spikes_indy_pili(start_dummy(sc),sc)))...      % earlier spike
                                    /(2*normy(sc)+(normy(sc)==0));
                            end
                        end
                    end
                    clear dummy start_dummy end_dummy
                end
                        ...
                        for trac2=trac1+1:d_para.num_trains
                            ...
                            for sc=1:r_para.run_pili_lengths(ruc)
                                m_res.pili_measures_mat(m_para.measure_indy(m_para.forward_spike_pili),pac,sc)= ...
                                    ...
                                    run_udists{start_dummy(sc),end_dummy(sc)}(foll_spikes_indy_pili(start_dummy(sc),sc)))...      % earlier spike
                                    /(2*normy(sc)+(normy(sc)==0));
                            end
                        end
                    end
            ...
            if f_para.select_measures(m_para.spike_pili) || f_para.select_measures(m_para.ri_spike_pili)             % SPIKE-pili and RI-SPIKE-pili
                if f_para.edge_correction
                    for trac=1:d_para.num_trains
                ...
                if f_para.select_measures(m_para.ri_spike_pili)  % RI-SPIKE-pili
                    if exist(['SPIKY_RI_SPIKE_MEX.',mexext],'file')
                        m_res.pili_measures_mat(m_para.measure_indy(m_para.ri_spike_pili),1:d_para.num_pairs,1:r_para.run_pili_lengths(ruc)) = ...
                            SPIKY_RI_SPIKE_MEX(int32(d_para.num_pairs),int32(r_para.run_pili_lengths(ruc)),int32(d_para.num_trains),...
                            foll_spikes_pili,prev_spikes_pili,int32(isi_indy_pili-1),int32(prev_spikes_indy_pili),int32(foll_spikes_indy_pili),...
                            ints,run_udists);
                    else
                        pac=0;
                        for trac1=1:d_para.num_trains-1
                            for trac2=trac1+1:d_para.num_trains
                                pac=pac+1;
                                m_res.pili_measures_mat(m_para.measure_indy(m_para.ri_spike_pili),pac,1:r_para.run_pili_lengths(ruc)) = ...
                                    ((run_udists{trac1,trac2}(prev_spikes_indy_pili(trac1,:)).*foll_spikes_pili(trac1,:)+...
                                    run_udists{trac1,trac2}(foll_spikes_indy_pili(trac1,:)).*prev_spikes_pili(trac1,:))./...
                                    ints(trac1,isi_indy_pili)+...
                                    (run_udists{trac2,trac1}(prev_spikes_indy_pili(trac2,:)).*foll_spikes_pili(trac2,:)+...
                                    run_udists{trac2,trac1}(foll_spikes_indy_pili(trac2,:)).*prev_spikes_pili(trac2,:))./...
                                    ints(trac2,isi_indy_pili))./...
                                    (ints(trac1,isi_indy_pili)+ints(trac2,isi_indy_pili));
                            end
                        end
                    end
                    ave_bi_vect(logical(m_para.select_bi_measures==m_para.ri_spike_pili),:)=...
                        ave_bi_vect(logical(m_para.select_bi_measures==m_para.ri_spike_pili),:)+...
                        sum((m_res.pili_measures_mat(m_para.measure_indy(m_para.ri_spike_pili),:,odds)+...
                        m_res.pili_measures_mat(m_para.measure_indy(m_para.ri_spike_pili),:,evens))/2.*...
                        repmat(shiftdim(m_res.isi(r_para.run_pico_starts(ruc):r_para.run_pico_ends(ruc)),-1),[1,d_para.num_pairs]),3);
                end                
            end            
        end
        ...
        if m_para.num_disc_measures>0                                                                                                   % disc
            ...
            if ruc==r_para.ruc_initial
                ...
                %m_res.all_pos_spikes=[zeros(d_para.num_trains,1) diff(all_cum_events,[],2) zeros(d_para.num_trains,1)];
                % position of events (position of following ISI)
                        ...
                        for trac2=trac1+1:d_para.num_trains
                            ...
                            if ruc==r_para.ruc_initial
                                ...
                                if num_pair_all_spikes(pac)>0
                                    ...
                                    if exist(['SPIKY_SPIKEsynchro_MEX789.',mexext],'file')  % ##### SPIKE-order not yet implemented in MEX, use Matlab-code #####
                                        if pac==1          % ########################## check spikes on edges and single spikes
                                            %spike_synchro=SPIKY_SPIKEsynchro_MEX2(int32(d_para.num_trains),int32(d_para.num_spikes),spikes);
                                            d_para.num_spikes=cellfun('length',spikes);
            ...
            if f_para.select_measures(m_para.isi_pico)                        % ISI (calculated first, then edge-correction of ints for SPIKE)
                if exist(['SPIKY_ISI_MEX.',mexext],'file')
                    ...
                    for trac1=1:d_para.num_trains-1
                        for trac2=trac1+1:d_para.num_trains
                            pac=pac+1;
                            start_dummy=find(ints(trac1,:)<ints(trac2,:));
                            m_res.pico_measures_mat(m_para.measure_indy(m_para.isi_pico),pac,start_dummy)=abs(ints(trac1,start_dummy)./ints(trac2,start_dummy)-1);
                            end_dummy=find(ints(trac1,:)>=ints(trac2,:) & ints(trac1,:)~=0);
                            m_res.pico_measures_mat(m_para.measure_indy(m_para.isi_pico),pac,end_dummy)=abs(ints(trac2,end_dummy)./ints(trac1,end_dummy)-1);
                        end
                    end
                end
            end
    ...
    if m_para.num_disc_measures>0
        if f_para.select_measures(m_para.spikesync_disc)
            if sum(num_pair_all_spikes)>0
                all_distances(m_para.disc_bi_measures_indy(m_para.measure_indy(m_para.spikesync_disc)))=...
                    sum(ave_bi_vect(m_para.disc_bi_measures_indy(m_para.measure_indy(m_para.spikesync_disc)),:))/sum(num_pair_all_spikes); % not simple average !!!
            else
                all_distances(m_para.disc_bi_measures_indy(m_para.measure_indy(m_para.spikesync_disc)))=1;
            end
            ave_bi_vect(m_para.disc_bi_measures_indy(m_para.measure_indy(m_para.spikesync_disc)),:)=...
                (ave_bi_vect(m_para.disc_bi_measures_indy(m_para.measure_indy(m_para.spikesync_disc)),:)+...
                (num_pair_all_spikes==0))./(num_pair_all_spikes+(num_pair_all_spikes==0));
        end
        if f_para.select_measures(m_para.spikeorder_disc)
            if sum(num_pair_all_spikes)>0
                all_distances(m_para.disc_bi_measures_indy(m_para.measure_indy(m_para.spikeorder_disc)))=...
                    sum(ave_bi_vect(m_para.disc_bi_measures_indy(m_para.measure_indy(m_para.spikeorder_disc)),:))/sum(num_pair_all_spikes); % not simple average !!!
            else
                all_distances(m_para.disc_bi_measures_indy(m_para.measure_indy(m_para.spikeorder_disc)))=0;
            end
            ave_bi_vect(m_para.disc_bi_measures_indy(m_para.measure_indy(m_para.spikeorder_disc)),:)=...
                (ave_bi_vect(m_para.disc_bi_measures_indy(m_para.measure_indy(m_para.spikeorder_disc)),:)+...
                (num_pair_all_spikes==0))./(num_pair_all_spikes+(num_pair_all_spikes==0));
        end
    end
    ...
    for mac=1:m_para.num_sel_bi_measures
                ...
                for sgc2=sgc:f_para.num_select_train_groups
                    if sgc~=sgc2
                        ...
                        if f_para.num_select_group_trains(sgc)>1
                            gmat(sgc,sgc2)=(sum(sum(imat(f_para.select_group_vect==f_para.select_train_groups(sgc),...
                                f_para.select_group_vect==f_para.select_train_groups(sgc))))-...
        ...
        if ismember(m_para.select_bi_measures(mac),m_para.disc_measures)
            if m_res.num_tmin_spikes>0 && m_res.num_tmax_spikes>0
                ...
                eval(['results.',char(m_para.all_measures_string(m_para.select_bi_measures(mac))),...
                    '.profile=disc_profiles(m_para.measure_indy(m_para.select_bi_measures(mac)),:)/(d_para.num_trains-1);'])
            elseif m_res.num_tmin_spikes>0
                ...
                eval(['results.',char(m_para.all_measures_string(m_para.select_bi_measures(mac))),...
                    '.profile=disc_profiles(m_para.measure_indy(m_para.select_bi_measures(mac)),1:end-1)/(d_para.num_trains-1);'])
            elseif m_res.num_tmax_spikes>0
                eval(['results.',char(m_para.all_measures_string(m_para.select_bi_measures(mac))),'.time=d_para.tmin+[cumsum(m_res.all_isi)];'])
                eval(['results.',char(m_para.all_measures_string(m_para.select_bi_measures(mac))),...
                    '.profile=disc_profiles(m_para.measure_indy(m_para.select_bi_measures(mac)),2:end)/(d_para.num_trains-1);'])
            else
                eval(['results.',char(m_para.all_measures_string(m_para.select_bi_measures(mac))),'.time=d_para.tmin+[cumsum(m_res.all_isi(1:end-1))];'])
                eval(['results.',char(m_para.all_measures_string(m_para.select_bi_measures(mac))),...
                    '.profile=disc_profiles(m_para.measure_indy(m_para.select_bi_measures(mac)),2:end-1)/(d_para.num_trains-1);'])
            end
    ...
    if m_para.memo_num_measures>0                                               % ##### pico & pili #####
        ...
        if r_para.num_runs>1 || num_profiles>0
            ...
            if m_para.num_pili_measures>0
                ...
                if f_para.select_measures(m_para.ri_spike_pili)
                    spike_diffs_ri_l=zeros(num_profiles,2*num_isi);
                end
        ...
        if r_para.num_runs==1
            if mod(f_para.profile_mode,2)==1                     % All
                ...
                if m_para.num_pili_measures>0
                    ...
                    if f_para.select_measures([m_para.ri_spike_pili])
                        spike_diffs_ri_l(1,1:2*num_isi)=mean(shiftdim(m_res.pili_measures_mat(m_para.measure_indy(m_para.ri_spike_pili),...
                            f_para.select_pairs,2*first_winspike_ind-1:2*last_winspike_ind),1),1);
                    end
                    if f_para.select_measures([m_para.realtime_spike_pili])
                ...
                for sgc=1:f_para.num_select_train_groups
                    if f_para.num_select_group_trains(sgc)>1
                        ...
                        if m_para.num_disc_measures>0
                            if f_para.select_measures(m_para.spikesync_disc)
                                if first_winspike_ind2==last_winspike_ind2
                                    ...
                                    elseif (win_cum_isi2(dummindy(1))~=sel_all_cum_isi2(1) || edge_spikes(2)) ...
                                            && f_para.profile_mode==2 && gsgz==mod(f_para.profile_mode,2)+1
                                    ...
                                    elseif (win_cum_isi2(dummindy(end))~=sel_all_cum_isi2(end) || edge_spikes(2)) && f_para.profile_mode==2 && ...
                                            gsgz==mod(f_para.profile_mode,2)+1
                        ...
                        if m_para.num_pili_measures>0
                            ...
                            if f_para.select_measures([m_para.ri_spike_pili])
                                spike_diffs_ri_l(gsgz,1:2*num_isi)=mean(shiftdim(m_res.pili_measures_mat(m_para.measure_indy(...
                                    m_para.ri_spike_pili),gm_indy,2*first_winspike_ind-1:2*last_winspike_ind),1),1);
                            end
                            if f_para.select_measures([m_para.realtime_spike_pili])
            ...
            if m_para.num_pili_measures>0
                if mod(edges,2)>0   % initial value in the middle of interspike interval
                    ...
                    if f_para.select_measures([m_para.ri_spike_pili])
                        spike_diffs_ri_l(:,1)=spike_diffs_ri_l(:,1)+(spike_diffs_ri_l(:,2)-spike_diffs_ri_l(:,1)).*...
                            repmat((f_para.tmin-m_res.pili_supi(first_pili_supi_ind-2))./...
                            (m_res.pili_supi(first_pili_supi_ind)-m_res.pili_supi(first_pili_supi_ind-2)),num_profiles,1);
                    end
                ...
                if mod(edges,4)>1   % final value in the middle of interspike interval
                    ...
                    if f_para.select_measures([m_para.ri_spike_pili])
                        spike_diffs_ri_l(:,2*num_isi)=spike_diffs_ri_l(:,2*num_isi-1)+(spike_diffs_ri_l(:,2*num_isi)-spike_diffs_ri_l(:,2*num_isi-1)).*...
                            repmat((f_para.tmax-m_res.pili_supi(last_pili_supi_ind))./...
                            (m_res.pili_supi(last_pili_supi_ind+2)-m_res.pili_supi(last_pili_supi_ind)),num_profiles,1);
                    end
                    if f_para.select_measures([m_para.realtime_spike_pili])   % linear approximation
            ...
            for ruc=min_ruc:max_ruc
                ...
                if mod(f_para.profile_mode,2)==1                     % All
                    ...
                    if m_para.num_pili_measures>0
                        ...
                        if f_para.select_measures([m_para.ri_spike_pili])
                            spike_diffs_ri_l(1,pili_prof_run_indy)=mean(shiftdim(m_res.pili_measures_mat(m_para.measure_indy(...
                                m_para.ri_spike_pili),f_para.select_pairs,pili_load_run_indy),1),1);
                        end
                        if f_para.select_measures(m_para.realtime_spike_pili)
                    ...
                    for sgc=1:f_para.num_select_train_groups
                        if f_para.num_select_group_trains(sgc)>1
                            ...
                            if m_para.num_disc_measures>0
                                ...
                                elseif (win_cum_isi2(prof_dummindy(1))~=sel_all_cum_isi2(1) || (edge_spikes(1) && ruc==1)) ...
                                        && f_para.profile_mode==2 && gsgz==mod(f_para.profile_mode,2)+1
                                    event_num=event_num+1;
                                end
                                ...
                                elseif (win_cum_isi2(prof_dummindy(end))~=sel_all_cum_isi2(end) ...
                                        || (edge_spikes(2) && ruc==1)) && f_para.profile_mode==2 && gsgz==mod(f_para.profile_mode,2)+1
                                    event_num=event_num+2;
                                end
                            end
                            if m_para.num_pico_measures>0
                            ...
                            if m_para.num_pili_measures>0
                                ...
                                if f_para.select_measures([m_para.ri_spike_pili])
                                    spike_diffs_ri_l(gsgz,pili_prof_run_indy)=mean(shiftdim(m_res.pili_measures_mat(...
                                        m_para.measure_indy(m_para.ri_spike_pili),gm_indy,pili_load_run_indy),1),1);
                                end
                                if f_para.select_measures(m_para.realtime_spike_pili)
        ...
        if mod(f_para.profile_mode,2)==1
            for trac=1:f_para.num_trains
                if ~isempty(spikes{trac})
                    ...
                    m_res.psth(1,1:f_para.psth_num_bins+1)=m_res.psth(1,1:f_para.psth_num_bins+1)+histcounts(spikes{trac},[bins inf]); 
                    %m_res.psth(1,1:f_para.psth_num_bins+1)=m_res.psth(1,1:f_para.psth_num_bins+1)+histcounts(spikes{trac},[bins 2*bins(end)-bins(end-1)]); 
            ...
            for sgc=1:f_para.num_select_train_groups
                if f_para.num_select_group_trains(sgc)>1
                    ...
                    for trac=find(f_para.select_group_vect==select_group)
                        ...
                        %m_res.psth(gsgz,1:f_para.psth_num_bins+1)=m_res.psth(gsgz,1:f_para.psth_num_bins+1)+histcounts(spikes{trac},bins);
                        % Matrix dimensions must agree.
                        %m_res.psth(gsgz,1:f_para.psth_num_bins+1)=m_res.psth(gsgz,1:f_para.psth_num_bins+1)+histcounts(spikes{trac},[bins 2*bins(end)-bins(end-1)]);
                        m_res.psth(gsgz,1:f_para.psth_num_bins+1)=m_res.psth(gsgz,1:f_para.psth_num_bins+1)+histcounts(spikes{trac},[bins inf]);
