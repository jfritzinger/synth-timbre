...
dataset=2;            % 1-Frequency mismatch,2-Spiking events,3-Splay state vs. identical,4:Test
...
para.select_measures      =[0 1 1 0 0 0 0 0];  % Select measures (0-calculate,1-do not calculate)
...
m_para.isi=1;
m_para.spike=[2 3 4 5];
m_para.spike_sync=[6 7];
m_para.psth=8;
...
if dataset==0                    % Frequency mismatch (from Fig. 2a of 2013 paper)
    %para.tmin=0.37; para.tmax=0.74; para.dts=10^-12;
    para.tmin=0; para.tmax=1; para.dts=10^-12;
    num_trains=2;
    spikes=cell(1,num_trains);
    spikes{1}=0.74;
    spikes{2}=0.37;
elseif dataset==1                    % Frequency mismatch (from Fig. 2a of 2013 paper)
    ...
    if num_plots>0
        ...
        for mc=1:length(measures)
            ...
            if mod(plotting,4)>1
                ...
                elseif ismember(measure,m_para.spike_sync) % Group profiles for SPIKE-Sync/SPIKE-order need extra treatment since support is different for each group
                    x=SPIKY_loop_results.(measure_var).time;
                    y=SPIKY_loop_results.(measure_var).profile;
                    hold on
