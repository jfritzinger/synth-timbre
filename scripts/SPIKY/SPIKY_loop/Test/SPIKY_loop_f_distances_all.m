...
if m_para.num_sel_bi_measures>0     % num_sel_measures>0
    ...
    for trac=1:d_para.num_trains
        ...
        %len=dummy2-dummy1+1-histcounts(empties,dummy1);
        len=dummy2-dummy1+1-histcounts(empties,[dummy1 2*dummy1(end)-dummy1(end-1)]);    
        ive{trac}=cumsum(len);
        ...
        if mod(f_para.profile_mode,2)==1
            for trac=1:f_para.num_trains
                if ~isempty(spikes{trac})
                    %m_res.psth(1,1:f_para.psth_num_bins+1)=m_res.psth(1,1:f_para.psth_num_bins+1)+histcounts(spikes{trac},bins);
