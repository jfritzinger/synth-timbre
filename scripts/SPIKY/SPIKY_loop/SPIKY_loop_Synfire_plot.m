function SPIKY_loop_Synfire_plot(spikes,para,handles)
...
plotting=127;  % +1:spikes,+2:dissimilarity profile,+4:dissimilarity matrix,+8:sorted spike trains,+16:sorted dissimilarity profile,...
               % +32:sorted dissimilarity matrix+64:surro_histo
...
if sync_thr>0
    ...
    for trc=1:para.num_trains
        ...
        spikes{trc}=or_spikes{trc}(trc_yes);
    end
    num_spikes=cellfun('length',spikes);
...
if exist(['SPIKE_order_sim_ann_MEX_alt.',mexext],'file')
    [st_indy_simann,A,total_iter]=SPIKE_order_sim_ann_MEX_alt(matr);           % ###################
else
