...
if ((run==1 && mod(plotting,2)>0) || (run==2 && mod(plotting,16)>7))
    ...
    if run==1
        title('Spike trains','Color','k','FontSize',13,'FontWeight','bold');
    else
        title('Sorted spike trains','Color','k','FontSize',13,'FontWeight','bold');
    end
    if run==2 || plotting<2
        xlabel('Time','Color','k','FontSize',13,'FontWeight','bold');
    end
    if mod(color_spikes,2)>0
        ...
        xl=xlim;
        yl=ylim;
        text(xl(1)+1.1*diff(xl),yl(2)-0.25*diff(yl),'D','Color','k','FontSize',12,'FontWeight','bold');
    ...
    if num_plots>1
        text(xl(1)-0.06*(xl(2)-xl(1)),yl(2)+0.05*(yl(2)-yl(1)),['(',alphabet(plot_pos(subplotc)),')'],'Color','k','FontSize',13,'FontWeight','bold');
    end
    box on
end
...
if ((run==1 && mod(plotting,4)>1) || (run==2 && mod(plotting,32)>15))
    ...
    if mod(plot_profiles,8)>3                                               % E-profile (symmetric, to be maximized)
        ...
        if ~isempty(e_prof)
            if plot_profiles==4
                para.synf_e_line_ls=':';
                plot(x,e_prof(1,:),'Color',para.synf_e_line_col,'LineStyle',para.synf_e_line_ls,'LineWidth',1,'Marker','.','MarkerSize',12);
            else
                para.synf_e_line_ls='-';
                plot(x,e_prof(1,:),'Color',para.synf_e_line_col,'LineStyle',para.synf_e_line_ls,'LineWidth',1,'Marker','.','MarkerSize',12);
        ...
        if run==1
            ...
            text(xl(2)+0.01*(xl(2)-xl(1)),yl(1)+(0.75-(prof_pos(prc2)-1)*0.25-0.25*(mod(plot_profiles,4)<=1))*(yl(2)-yl(1)),[prof_letter(3),...
                '_u=',num2str(mean_thr,3)],'Color',para.synf_f_result_col,'FontSize',13,'FontWeight','bold');
        else
            %text(xl(2)+0.01*(xl(2)-xl(1)),yl(1)+(0.75-(prof_pos(prc2)-1)*0.25-0.25*(mod(plot_profiles,4)<=1))*(yl(2)-yl(1)),[prof_letter(3),'_s=',...
            %num2str(mean_thr,3)],'Color',prof_color(1),'FontSize',fs+1,'FontWeight','bold')
            text(xl(2)+0.01*(xl(2)-xl(1)),yl(1)+(0.75-(prof_pos(prc2)-1)*0.25-0.25*(mod(plot_profiles,4)<=1))*(yl(2)-yl(1)),...
                [prof_letter(3),'_s=',num2str(mean_thr,3)],'Color',para.synf_f_result_col,'FontSize',13,'FontWeight','bold');
        end
    end
    ...
    if mod(plot_profiles,2)>0                                               % SPIKE-Sync-profile C (symmetric envelope)
        ...
        if ~isempty(c_prof)
            if plot_profiles==1
                ...
                para.synf_c_line_ls='--';
                synf_c_line_lh(run*2-1)=plot(x,c_prof(1,:),'Color',para.synf_c_line_col,'LineStyle',para.synf_c_line_ls,'LineWidth',1,...
                    'Marker','.','MarkerSize',13);
                synf_c_line_lh(run*2)=plot(x,-c_prof(1,:),'Color',para.synf_c_line_col,'LineStyle',para.synf_c_line_ls,'LineWidth',1,...
                    'Marker','.','MarkerSize',13);
        ...
        text(xl(2)+0.01*(xl(2)-xl(1)),yl(1)+(0.75-(prof_pos(prc2)-1)*0.25)*(yl(2)-yl(1)),[prof_letter(1),'=',num2str(mean_y3,3)],...
            'Color',para.synf_c_result_col,'FontSize',13,'FontWeight','bold');
    ...
    if plotting<4
        xlabel('Time',...
            'Color','k','FontSize',para.synf_label_fs,'FontWeight',para.synf_label_fw);
    end
    if run==1
        title(['Time profile ',prof_letter(4)],'Color','k','FontSize',13,'FontWeight','bold');
    else
        title(['Time profile ',prof_letter(4),' for sorted spike trains'],'Color','k','FontSize',13,'FontWeight','bold');
    end
    if mod(color_spikes,2)>0
    ...
    if num_plots>1
        ...
        if run==1 && mod(plotting,8)>5    % First matrix
            text(xl(1)-0.06*(xl(2)-xl(1)),yl(1)-0.4*(yl(2)-yl(1)),['(',alphabet(plot_pos(subplotc)+1),')'],'Color','k','FontSize',13,'FontWeight','bold');
        elseif run==2 && num_plots==6     % Second matrix
            text(xl(1)-0.06*(xl(2)-xl(1)),yl(2)+1.4*(yl(2)-yl(1)),['(',alphabet(plot_pos(subplotc)-1),')'],'Color','k','FontSize',13,'bold');
        end
    end
    box on
end
...
if (run==1 && mod(plotting,8)>3) || (run==2 && mod(plotting,64)>31)
    ...
    if run==1
        title(['Pairwise matrix ',prof_letter(2)],'Color','k','FontSize',13,'FontWeight','bold');
    else
        synf_title_fh(4)=title(['Sorted pairwise matrix ',prof_letter(2)],'Color','k','FontSize',13,'FontWeight','bold');
    end
    ...
    if mark_upper_matrix_half==1
        for trc=1:para.num_trains-1
            synf_halfmat_line_lh(run,trc*2-1)=line((trc+0.5)*ones(1,2),[trc-0.5 trc+0.5],'Color','k','LineStyle','-','LineWidth',1);
            line([trc+0.5 trc+1.5],(trc+0.5)*ones(1,2),'Color','k','LineStyle','-','LineWidth',1);
            
            %line((trc+0.5)*ones(1,2),[trc-0.5 trc+0.5],'Color','k','LineWidth',2)
            %line([trc+0.5 trc+1.5],(trc+0.5)*ones(1,2),'Color','k','LineWidth',2)
        end
        line((para.num_trains+0.5)*ones(1,2),[0.5 para.num_trains-0.5],'Color','k','LineStyle','-','LineWidth',1);
        line([1.5 para.num_trains+0.5],0.5*ones(1,2),'Color','k','LineStyle','-','LineWidth',1);
    ...
    if num_surros>0
        ...
        if mod(plotting,128)>63
            ...
            if show_z_scores==1
                ...
                line((mean_surros+std_surros)*ones(1,2),yl(1)+[0.6 0.7]*(yl(2)-yl(1)),'Color',cols(1),'LineWidth',3)
                if pval<=0.05
                    title(['z = ',num2str(z_score,3),'  ;  p = ',num2str(pval,2),'**'],'Color','k','FontSize',13,'FontWeight',1);
                elseif pval<0.3
                    title(['z = ',num2str(z_score,3),'  ;  p > 0.05'],'Color','k','FontSize',13,'FontWeight',1);
                else
                    title(['z = ',num2str(z_score,3),'  ;  p >> 0.05'],'Color','k','FontSize',13,'FontWeight',1);
                end
            else
                if pval<=0.05
                    title(['Significance:  p = ',num2str(pval,2),'**'],'Color','k','FontSize',13,'FontWeight',1);
                elseif pval<0.3
                    title('Significance:  p > 0.05','Color','k','FontSize',13,'FontWeight',1);
                else
                    title('Significance:  p >> 0.05','Color','k','FontSize',13,'FontWeight',1);
                end
            end
            line(fin*ones(1,2),yl,'Color',cols(2),'LineStyle','--','LineWidth',3)
            xlabel('F','Color','k','FontSize',13,'FontWeight','bold');
            ylabel('#','Color','k','FontSize',13,'FontWeight','bold');
            axis square
            if any(mod(get(gca,'YTick'),1)>0)
                set(gca,'YTick',0:max(histo))
            end
            set(gca,'FontSize',fs-3)
            text(xl(1)-0.5*(xl(2)-xl(1)),yl(2)+0.05*(yl(2)-yl(1)),['(',alphabet(6),')'],'Color','k','FontSize',13,'FontWeight','bold');
        end
