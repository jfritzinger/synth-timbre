...
e_para=struct('event_type',2,'movave',0,'movave_order',3,'max_window',2,'max_elevation',0.01,'min_window',2,'min_elevation',0.01,...
    ...
    'xhisto_all_trials',1,'num_trials',2,'xhisto_equi_custom',10,'xhisto_num_bins',10,'xhisto_edges',[256 2.5*256 4*256 5.5*256 7*256 8.5*256 10*256],...
    'xhisto_grand_average',1,'threshold',0.8,'type',1,'MC',[10 11 14 15 16 19 20],'nonMC',[1:9 12 13 17 18 21:30]);
...
EVD_parameter_panel=uipanel('units','normalized','position',[0.5 0.12 0.48 0.86],'Title','Parameters','FontSize',15,'FontWeight','bold',...
    'parent',EVD_events_panel);

EVD_movave_checkbox=uicontrol('style','checkbox','units','normalized','position',[0.06 0.9 0.46 0.05],'string','Moving averages',...
    'HorizontalAlignment','left','FontSize',14,'FontUnits','normalized','parent',EVD_parameter_panel,'callback',{@EVD_movave_callback},...
    'Value',e_para.movave);
...
uicontrol('style','pushbutton','units','normalized','position',[0.075 0.03 0.25 0.06],'string','Back',...
    'FontSize',15,'FontUnits','normalized','FontWeight','bold','BackgroundColor',[0.8353 0.8235 0.7922],'parent',EVD_events_panel,...
    'CallBack',{@EVD_Back_callback});
EVD_Update_pushbutton=uicontrol('style','pushbutton','units','normalized','position',[0.675 0.03 0.25 0.06],'string','Update',...
    'FontSize',15,'FontUnits','normalized','FontWeight','bold','BackgroundColor',[0.8353 0.8235 0.7922],'parent',EVD_events_panel,...
    'CallBack',{@EVD_Update_callback});
...
set(EVD_UserData.fh,'Units','Normalized','Userdata',EVD_UserData)
    ...
    function EVD_plot_events(varargin)        
        ...
        elseif e_para.event_plot==3
            for efc=1:EVD_UserData.num_total_events
                EVD_UserData.eh(efc)=line(max([c(efc)-e_para.event_precol(ec) 1]):min([c(efc)+e_para.event_postcol(ec) EVD_UserData.num_samples]),...
                    EVD_UserData.num_channels-r(efc)+EVD_UserData.plot_data(r(efc),max([c(efc)-e_para.event_precol(ec) 1]):min([c(efc)+...
                    e_para.event_postcol(ec) EVD_UserData.num_samples]))*EVD_UserData.yscale,'color',e_para.event_color(ec),...
                    'LineWidth',e_para.event_linewidth(ec));
            end
        end
        ...
        if e_para.event_type==3
            ...
            for rc=1:EVD_UserData.num_channels
                EVD_UserData.tlh(rc)=line([0 EVD_UserData.smax-EVD_UserData.smin],(EVD_UserData.num_channels-rc+...
                    (e_para.crossthreshold-EVD_UserData.plot_min(rc))/(EVD_UserData.plot_max(rc)-EVD_UserData.plot_min(rc))*EVD_UserData.yscale)*ones(1,2),...
        ...
        if e_para.xhisto_equi_custom==1                                              % X-Histo
            ...
            set(EVD_UserData.hh2,'YData',(-EVD_UserData.histo_y(1:e_para.xhisto_num_bins)/...
                (max(EVD_UserData.histo_y(1:e_para.xhisto_num_bins))+(max(EVD_UserData.histo_y(1:e_para.xhisto_num_bins))==0))*...
                0.2*EVD_UserData.num_channels))
        ...
        EVD_UserData.th(EVD_UserData.num_channels+2)=text(xl(2)-0.11*(xl(2)-xl(1)),yl(1)+0.08*(yl(2)-yl(1)),num2str(EVD_UserData.num_total_events),...
            'FontSize',15,'FontWeight','bold');        
        EVD_UserData.th(EVD_UserData.num_channels+3)=text(xl(2)-0.15*(xl(2)-xl(1)),yl(1)+0.015*(yl(2)-yl(1)),num2str(max(EVD_UserData.histo_y)),...
            'FontSize',13);        
